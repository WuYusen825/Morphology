...
for ph,forms in SERIES.items():
    pat=re.compile(('从[^，。]{1,3}[，、]?' if ph=='其' else '')+'(['+forms+'])(省)?(亦)?聲')
    ...
    for d,rel in cands:
        ...
        rows.append(dict(group='control' if ph in CONTROL else 'youwen',series=ph,char=d['wordhead'],sw_id=d['id'],relation=rel,sw_gloss=d['explanation'],dx_fanqie=d['pronunciation'],
